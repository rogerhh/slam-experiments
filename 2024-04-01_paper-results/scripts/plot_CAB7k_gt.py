...
if __name__ == "__main__":
    ...
    plt.rcParams.update({'font.size': 20, 'axes.titlesize': "medium", 'axes.labelsize': 'medium'})
    ...
    for xs, ys, zs in zip(xss, yss, zss):
        ...
        alpha = 0.8
        if is3D:
            ax.plot(xs, ys, zs, linewidth=width, alpha=alpha)
        else:
            ax.plot(xs, ys, linewidth=width, alpha=alpha)
